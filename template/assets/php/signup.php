...
if(!filter_var($e, FILTER_VALIDATE_EMAIL))
  ...
  {
	$url = $GryphonStorageURL . "newuser.php?a=" . $a . "&j=" . $j . "&p=" . $p . "&e=" . $e;
	...
	$key = hash("sha256",$key);
	$encrypted = trim($page);
	if (($encrypted == "account exists") || ($encrypted == "fail")) 
	{
		$decrypted = $encrypted;
	}
	else
	...
	$answer = "fail";
	if (($sendgrid_api_user == "your_sendgrid_username") || ($sendgrid_api_key == "your_sendgrid_password") || ($decrypted == "fail") || ($decrypted == "account exists")){}
		else
		{
			...
			$sendgrid = $sendgrid . 	"&subject=" . urlencode($signup_sendgrid_subject);
			$sendgrid = $sendgrid . 	"&html=" . urlencode($signup_sendgrid_html_before_password . "<br><br>Password: " . $decrypted . "<br><br>" . $signup_sendgrid_html_after_password);
			$sendgrid = $sendgrid . 	"&from=" . $signup_sendgrid_from;
